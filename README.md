<div align="center">

  <img src="assets/banner.svg" alt="Nyla Rivarde — Business Analyst" width="100%" />

  <br>

  ### Business Analysis · Product · Process Improvement · Data

</div>

<br>

<!-- TODO: replace # with confirmed LinkedIn URL before publishing -->

## About Me

I work in commercial insurance underwriting and I'm completing my MBA in Data Analytics at LSU. I'm building on that experience to pursue Business Analyst, Product, and data-oriented roles, with a focus on simplifying processes and helping people make better decisions.

## Core Capabilities

- **Business Analysis** — requirements, process mapping, stakeholder needs
- **Product Thinking** — scoping, prioritization, MVP reasoning
- **Process Improvement** — finding friction, simplifying workflows
- **Data Analysis** — turning operational data into decisions
- **UX-Oriented Problem Solving** — designing for how people actually work
- **AI-Enabled Workflows** — using AI tools to support analysis and product development

## Quick Access

[![Portfolio](https://img.shields.io/badge/Portfolio-Underwriter360-3E5FEB?style=flat-square)](https://github.com/NylaRivarde/underwriter360)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-3E5FEB?style=flat-square&logo=linkedin&logoColor=white)](#)

## Featured Projects

| | Project | What It Shows |
|---|---|---|
| **01** | [**Underwriter360**](https://github.com/NylaRivarde/underwriter360) — Commercial Property underwriting workflow visibility and prioritization concept | Business Analysis · Product · Process · Data |
| **02** | *Coming Soon* | — |

## Tools & Methods

**Analysis & Documentation** — Requirements & User Stories · Process Mapping · Acceptance Criteria
**Data & Reporting** — Excel · Power BI
**Delivery** — Jira · Agile/Scrum
**Analysis Support** — AI-assisted analysis & prototyping

**Certifications:** CSPO — Certified Scrum Product Owner · Jira Essentials

## Let's Connect

📫 [LinkedIn](#)
